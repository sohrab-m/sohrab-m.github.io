...
%
% Generates Figure 4. Contains random functions, expect values to vary. If
...
f_sampled = sin(X_sampled)+normrnd(0, sigma_noise, 1, n_samples); % model is a sin funciton


K_xx = cov_se(X_sampled, X_sampled)+ sigma_noise^2*eye(n_samples);
...
ylim([-5, 5]);
%plot(X_true, f_true, 'k--', 'linewidth', 0.5);
plot(X_sampled, f_sampled(1, :), 'k+', 'markersize', 20, 'LineWidth', 2);
...
hold on
plot(X_true, f_true, 'k--', 'linewidth', 0.5);
plot(X_true, mu_new, 'r-');
plot(X_sampled, f_sampled(1, :), 'k+', 'markersize', 20, 'LineWidth', 2);
legend('\pm 2sigma', 'sin(x)', '\mu_{f_*}');
xlabel('input, x');
ylabel('output, f(x)');
ylim([-5, 5]);
